 ...
 ***************************************************************************************************/
	var Validator = {
		...
		_handleError : function(message, el) {
			...
			el.style.backgroundColor="#FFD8D8";
			el.focus();
		...
		"validate-kor" : function(v) {
			return /[^ ㄱ-ㅣ가-힣]/.test(v) ? "한글만 입력하셔야 합니다." : false;
		},
		// 특수문자 확인
		"validate-special-text" : function(v) {
			return /[~!@\#$%<>^&*\()\-=+_\’]/gi.test(v) ? "특수문자없이 입력하셔야 합니다." : false;
		},
		// 영문, 숫자, _(언더바) 만  입력
		...
		"validate-digits-comma" : function(v) {
			return !/(^[0-9\,]+$)/.test(v) ? "숫자,(콤마) 만  입력하셔야 합니다." : false;
		},
		// 영문과 숫자만 입력
		...
		"validate-url" : function(v) {
			if(v) {
				return !/^(http|https|ftp):\/\/(([A-Z0-9][A-Z0-9_-]*)(\.[A-Z0-9][A-Z0-9_-]*)+)(:(\d+))?\/?/i.test(v) ? "http://를 포함한 바른 URL을 입력하셔야 합니다." : false;
			}
			return false;
		},
