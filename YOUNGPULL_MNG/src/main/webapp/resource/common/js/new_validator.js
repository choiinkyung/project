	...
	var Validator = {
		...
		_handleError : function(message, el) {
			...
			var title = Validator._getTitle(el);
				if(title != "") {
					document.getElementById("message_"+el.id).innerHTML = title +"(을)를 " + message;
				}else{
					document.getElementById("message_"+el.id).innerHTML = message;
				}
		...
		//영문 필수입력 추가
		"eng-required" : function(v) {
			return !v ? " is required" : false;
		},
		"eng-validate-email" : function(v) {
			if(v == "") return false;
			return !/\w{1,}[@][\w\-]{1,}([.]([\w\-]{1,})){1,3}$/.test(v) ? " You must enter a valid e-mail address" : false;
		},
		// 필수입력
		"required" : function(v) {
			return !v ? "입력 해 주세요." : false;
		},
		// white space 없이 입력
		"validate-white-space" : function(v) {
			return /\s+/.test(v) ? "공백없이 입력 해 주세요." : false;
		},
		// 숫자만 입력
		"validate-digits" : function(v) {
			return isNaN(v) || /[^\d]/.test(v)? "숫자만 입력 해 주세요." : false;
		},
		// 영문으로 시작 입력
		"validate-alpha" : function(v) {
			return /[^a-zA-Z]+$/.test(v) ? "알파뱃으로 시작되도록 입력 해 주세요." : false;
		},
		// 한글만 입력
		"validate-kor" : function(v) {
			return /[^ ㄱ-ㅣ가-힣]/.test(v) ? "한글만 입력 해 주세요." : false;
		},
		// 특수문자 확인
		"validate-special-text" : function(v) {
			return /[~!@\#$%<>^&*\()\-=+_\’]/gi.test(v) ? "특수문자없이 입력 해 주세요." : false;
		},
		// 영문, 숫자, _(언더바) 만  입력
		"validate-basic-char" : function(v) {
			return !/[a-zA-Z0-9_]/.test(v) ? "알파뱃, 숫자, _(언더바) 만 입력 해 주세요." : false;
		},
		// 숫자,(콤마) 만  입력
		"validate-digits-comma" : function(v) {
			return !/(^[0-9\,]+$)/.test(v) ? "숫자,(콤마) 만  입력 해 주세요." : false;
		},
		// 영문과 숫자만 입력
		"validate-alpha-num" : function(v) {
			return /\W/.test(v) ? "알파뱃과 숫자만 입력 해 주세요." : false;
		},
		// 날자 사용가능한 숫자 입력
		"validate-date" : function(v) {
			var date = new Date(v);
			return isNaN(v) ? "바른 날짜를 입력 해 주세요." : false;
		},
		// 유효한 email 입력
		"validate-email" : function(v) {
			if(v == "") return false;
			return !/\w{1,}[@][\w\-]{1,}([.]([\w\-]{1,})){1,3}$/.test(v) ? "바른 이메일 주소를 입력 해 주세요." : false;
		},
		// 유효한 URL 입력
		"validate-url" : function(v) {
			if(v) {
				return !/^(http|https|ftp):\/\/(([A-Z0-9][A-Z0-9_-]*)(\.[A-Z0-9][A-Z0-9_-]*)+)(:(\d+))?\/?/i.test(v) ? "http://를 포함한 바른 URL을 입력 해 주세요." : false;
			}
			return false;
		},
		//비밀번호 글자수 체크
		"validate-password-length" : function(v) {
        ...
        "validate-tel-num" : function(v, elm, param) {
        	if(v) {
        		...
        		for(i = 0; i < tmpArray.length; i++ ) {
        			if(tmpArray[i] == "") return "정확히  입력해 주세요.";
        		}
        	}
        ...
        // 웹상에서 사용가능한  엑셀 파일 종류인지 확인한다. xls, xlsx 만 인정함
        "validate-is-excel" : function(v) {
        	if(v) {
        		var ext = "";
        		if(v.indexOf(".") >= 0) {
        			ext = v.substring(v.lastIndexOf(".")+1);
        			ext = ext.toLowerCase();
        			if(ext.length != 3 || !(ext == "xls" || ext == "xlsx"))
        				return "파일은 xls, xlsx 파일만 등록을 하실수 있습니다.";
        		} else {
        			return "파일의 확장자가 없습니다.";
        		}
        	}
        ...
        "validate-is-media" : function(v) {
        	if(v) {
        		var ext = "";
        		if(v.indexOf(".") >= 0) {
        			ext = v.substring(v.lastIndexOf(".")+1);
        			ext = ext.toLowerCase();
        			if(ext.length != 3 || !(ext == "avi" || ext == "mp4"))
        				return "파일은 avi, mp4 파일만 등록을 하실수 있습니다.";
        		} else {
        			return "파일의 확장자가 없습니다.";
        		}
        	}
